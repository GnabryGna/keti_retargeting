...
class DualArmEnv:
    ...
    def step(self, action):
        ...
        self.left_robot_hand.servo_joint(action["left_hand_qpos"])
    ...
    def get_observation(self):
        ...
        left_robot_hand_q_pos = (
            self.left_robot_hand.get_joint_pos()
        ...
        contact_data = []
        for id, contact in enumerate(self.data.contact):
            contact_force = np.zeros(6)
            mujoco.mj_contactForce(self.model, self.data, id, contact_force)

            data = {}
            data['pos'] = contact.pos.copy()
            data['frame'] = contact.frame.copy()
            data['friction'] = contact.friction.copy()
            data['mu'] = contact.mu
            data['dim'] =  contact.dim
            data['geom1'] = contact.geom[0]
            data['geom2'] = contact.geom[1]
            data['contact_force'] = contact_force
    ...
    def get_reward(self):
        ...
        if (
            self.target_ycb_x is not None
            or self.target_ycb_z is not None
